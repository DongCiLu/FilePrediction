...
def main():
    ...
    #num_batches = 10
    #print dev_weights
    [results, final_weights] = test_crush(crush, args['min_x'], args['max_x'], args['num_replicas'], dev_weights, args['bucket_down_ratio'], args['dev_down_ratio'], args['num_batches'])
    print 'times each device has been chosen: '+str(results)
    f4 = open('sim_results.txt', 'w')
    for i in range(len(results)):
        f4.write(str(i+1)+' '+str(final_weights[i])+' '+str(results[i])+'\n')
    f4.close()
    print 'done!'
